...
/** @scope DragList.dragController.prototype */ {
  ...
  */
  collectionViewComputeDragOperations: function(view, drag, proposedDragOps) {
    if (drag.hasDataType(DragList.Employee)) {
      ...
      if (source && source.delegate === this) return proposedDragOps || SC.DRAG_MOVE;

    } else return SC.DRAG_NONE;
  },

  collectionViewValidateDragOperation: function(view, drag, dragOp, proposedInsertionIdx, proposedDropOp) {
    return SC.DRAG_ANY;
  },
  ...
  */
  collectionViewPerformDragOperation: function(view, drag, dragOp, idx, dropOp) {
    ...
    if (!(dragOp & SC.DRAG_MOVE)) {
      ret = SC.DRAG_COPY;
    } else {
      var otherContent = source.get('content');
      if (otherContent) {
        otherContent.removeObjects(employees);
        ret = SC.DRAG_MOVE;
      }
    }
